...
operations_to_apply = dict(
                    ...
                    CorrectEvents = 1,           # *** MUST ALWAYS BE SET to 1! To correct for stim delay
                    ...
                    PlotEoGEpochs_Manual = 0,    # You need to set all Epochs parameters below in section 2, Epoch. Recommend to use Auto instead.
                    ...
                    ## PCA
                    ProcessPCA = 1,              # *** If running any PCA functions below, this should always be set to 1.
                    PlotPCACompo = 0,            # Plot PCA noise Component for EEG, Mag and Grad based on EoG and ECG.
                    ApplyPCA = 1,                # *** Apply PCA output onto epoch data. ProcessPCA MUST be set to 1 for this to work. Should also be set to 1 as it overwrites old epoch data with the new projection.
                    ...
                    ApplyProj = 1,               # *** Apply PCA projections on ERP. 
                    PlotProj = 0,
                    PlotTopo = 0,
                    ...
                    Contrast = 0,                # Shows contrast activity over time between 2 conditions. Need to define various variables in Section 2 - Contrast, part 1
                    Topo = 0,                    # Shows topographic activity morphing over time on a sensor group defined by the 'meg' variable in Section 2 - Evoked. Also need to specifiy settings in Section 2 - Contrasts, part 2
                    SaveTopo = 0,                # Saves Topo plot
                    AllTopo = 0,                 # Plots all sensors activity of all conditions in a single topographic plot. Need to specify time range in Section 2 - Contrasts, part 3
                    CompareEvokeds = 0           # Compare evoked activity across all conditions on a specific sensor. Sensor needs to be defnied in Section 2 - Contrast, part 4
